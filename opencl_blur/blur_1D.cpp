...
DCT::setupCL(void)
{
    ...
    // getting device on which to run the sample
       std::cout << "will " <<  sampleArgs->deviceId << std::endl;
    status = getDevices(context, &devices, sampleArgs->deviceId,
                        sampleArgs->isDeviceIdEnabled());
	std::cout << sampleArgs->deviceId << std::endl;
    //status = setDeviceInfo(sampleArgs->deviceId);
    /*        size_t tempSize = 0;
            char *name;
...
*/
    {
        ...
        cl_command_queue_properties prop = CL_QUEUE_PROFILING_ENABLE;//0;
        commandQueue = clCreateCommandQueue(
                           context,
                           devices[sampleArgs->deviceId],
                           prop,
                           &status);
        //std::cout << "Testing of clCreateCommandQueue : " << status << std::endl;
...
DCT::runCLKernels(void)
{
    ...
    CHECK_OPENCL_ERROR(status, "clGetEventProfilingInfo failded.");
    clGetEventProfilingInfo(ndrEvt, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &endtime1, NULL);
    time_spent1 = endtime1 - starttime1;
    //printf("First Kernel time using clGetEventProfilingInfo : %llu \n", time_spent1);
    std::cout << "First Kernel using clGetEventProfilingInfo: " << time_spent1 << std::endl;

    //gettimeofday(&t2, NULL);
    unsigned int t = (t2.tv_sec - t1.tv_sec) * 1000000 + (t2.tv_usec - t1.tv_usec);
    printf("First Kernel : EnqueueNDRange: %u\n", t);
     ...
     */
    cl_event ndrEvt2;
    cl_ulong starttime2=0.0, endtime2=0.0, time_spent2=0.0;
    gettimeofday(&t3, NULL);
    status = clEnqueueNDRangeKernel(
                 commandQueue,
                 kernel,
    ...
    CHECK_OPENCL_ERROR(status, "clGetEventProfilingInfo failded.");
    clGetEventProfilingInfo(ndrEvt2, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &endtime2, NULL);
    time_spent2 = endtime2 - starttime2;
    //printf("First Kernel time using clGetEventProfilingInfo : %llu \n", time_spent1);
    std::cout << "Second Kernel using clGetEventProfilingInfo: " << time_spent2 << std::endl;

    //gettimeofday(&t4, NULL);
    t = (t4.tv_sec - t3.tv_sec) * 1000000 + (t4.tv_usec - t3.tv_usec);
    printf("Second Kernel : EnqueueNDRange: %u\n", t);
#endif
    // Enqueue readBuffer
    cl_event readEvt;
